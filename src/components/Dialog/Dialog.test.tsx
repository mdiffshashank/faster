...
function ControlledDialog(props: Omit<React.ComponentProps<typeof Dialog>, "open" | "onClose"> & {
  initialOpen?: boolean;
}) {
...
describe("Dialog", () => {
  ...
  describe("rendering", () => {
    it("does NOT render when open=false", () => {
      render(<Dialog open={false} title="Test" onClose={jest.fn()}>Body</Dialog>);
      expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    });

    it("renders when open=true", () => {
      render(<Dialog open={true} title="Test" onClose={jest.fn()}>Body</Dialog>);
      expect(screen.getByRole("dialog")).toBeInTheDocument();
    });

    it("renders the title text", () => {
      render(<Dialog open={true} title="My Dialog Title" onClose={jest.fn()}>Body</Dialog>);
    ...
    it("does NOT render footer section when footer prop is omitted", () => {
      render(<Dialog open={true} title="Title" onClose={jest.fn()}>Body</Dialog>);
  ...
  describe("accessibility attributes", () => {
    ...
    it("has aria-labelledby pointing to the title heading", () => {
      render(<Dialog open={true} title="Labelled Dialog" onClose={jest.fn()}>Body</Dialog>);
  ...
  describe("close behaviour", () => {
    it("calls onClose when close button is clicked", async () => {
      ...
      render(<Dialog open={true} title="Test" onClose={onClose}>Body</Dialog>);
    ...
    it("does NOT call onClose when clicking inside the dialog panel", async () => {
      const onClose = jest.fn();
      render(<Dialog open={true} title="Test" onClose={onClose}>Body text</Dialog>);
  ...
  describe("sizes", () => {
    it.each(["sm", "md", "lg"] as const)("renders %s size without errors", (size) => {
      render(<Dialog open={true} size={size} title="Test" onClose={jest.fn()}>Body</Dialog>);
      expect(screen.getByRole("dialog")).toBeInTheDocument();
    });

    it("applies max-w-sm class when size=sm", () => {
      render(<Dialog open={true} size="sm" title="Test" onClose={jest.fn()}>Body</Dialog>);
      expect(screen.getByRole("dialog")).toHaveClass("max-w-sm");
    });

    it("applies max-w-lg class when size=lg", () => {
      render(<Dialog open={true} size="lg" title="Test" onClose={jest.fn()}>Body</Dialog>);
